...
# --------------------------------------------------------
# 在文件顶部添加导入
from .swin_mamba import SwinMamba
...
def build_model(config, is_pretrain=False):
    ...
    if model_type == 'swin':
        ...
        model = SwinMLP(img_size=config.DATA.IMG_SIZE,
                        ...
                        use_checkpoint=config.TRAIN.USE_CHECKPOINT)
    # 在build_model函数中添加SwinMamba的构建逻辑
    elif model_type == 'swin_mamba':
        model=SwinMamba(img_size=config.DATA.IMG_SIZE,
                        patch_size=config.MODEL.SWIN_MAMBA.PATCH_SIZE,
                        in_chans=config.MODEL.SWIN_MAMBA.IN_CHANS,
                        num_classes=config.MODEL.NUM_CLASSES,
                        embed_dim=config.MODEL.SWIN_MAMBA.EMBED_DIM,
                        depths=config.MODEL.SWIN_MAMBA.DEPTHS,
                        num_heads=config.MODEL.SWIN_MAMBA.NUM_HEADS,
                        window_size=config.MODEL.SWIN_MAMBA.WINDOW_SIZE,
                        mlp_ratio=config.MODEL.SWIN_MAMBA.MLP_RATIO,
                        qkv_bias=config.MODEL.SWIN_MAMBA.QKV_BIAS,
                        qk_scale=config.MODEL.SWIN_MAMBA.QK_SCALE,
                        drop_rate=config.MODEL.DROP_RATE,
                        drop_path_rate=config.MODEL.DROP_PATH_RATE,
                        ape=config.MODEL.SWIN_MAMBA.APE,
                        patch_norm=config.MODEL.SWIN_MAMBA.PATCH_NORM,
                        use_checkpoint=config.TRAIN.USE_CHECKPOINT,
                        fused_window_process=config.MODEL.SWIN_MAMBA.FUSED_WINDOW_PROCESS,
                        d_state=config.MODEL.SWIN_MAMBA.D_STATE,
                        rms_norm=config.MODEL.SWIN_MAMBA.RMS_NORM)
