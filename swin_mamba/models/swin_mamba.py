...
class SwinMambaBlock(nn.Module):
    ...
    Args:
        ...
        num_heads (int): Number of attention heads (保留以兼容性).
        window_size (int): Window size.
        ...
        d_state (int): Mamba的状态维度. Default: 16
        rms_norm (bool): 是否使用RMSNorm. Default: False
    """
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.num_heads = num_heads  # 保留以兼容性
        ...
        if self.shift_size > 0:
            # calculate attention mask for SW-MSA (保留以兼容性)
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                ...
                x = WindowProcessReverse.apply(mamba_windows, B, H, W, C, self.shift_size, self.window_size)
        else:
            shifted_x = window_reverse(mamba_windows, self.window_size, H, W)  # B H' W' C
            x = shifted_x
        x = x.view(B, H * W, C)
...
class SwinMamba(nn.Module):
    r""" Swin Mamba
        将Swin Transformer中的WindowAttention替换为Mamba模块
    ...
    Args:
        ...
        num_heads (tuple(int)): Number of attention heads in different layers (保留以兼容性).
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 d_state=16, rms_norm=False, **kwargs):
        super().__init__()
...
class SwinMamba(nn.Module):
    r""" Swin Mamba
        A PyTorch impl of Swin Transformer with Mamba blocks
